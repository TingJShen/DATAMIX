...
class ActorRolloutRefWorker(Worker):
    ...
    @register(dispatch_mode=Dispatch.DP_COMPUTE_PROTO)
    def compute_next_token_embeddings(self, data: DataProto):
        assert self._is_actor
        if self._is_offload_param:
            load_fsdp_model_to_gpu(self.actor_module_fsdp)
        ...
        with torch.no_grad():
            ...
            if "position_ids" in data.batch.keys():
                model_kwargs["position_ids"] = data.batch["position_ids"]

            captured_last_hidden_state = {}
            base_module = getattr(getattr(self, "actor_module", None), "model", None)
            if base_module is None:
                wrapped_module = getattr(self.actor.actor_module, "_fsdp_wrapped_module", None)
                if wrapped_module is None:
                    wrapped_module = getattr(self.actor.actor_module, "module", None)
                base_module = getattr(wrapped_module, "model", None)
            if base_module is None:
                base_module = getattr(self.actor.actor_module, "model", None)

            hook_handle = None
            if base_module is not None:
                def _capture_base_model_output(_module, _inputs, hook_output):
                    last_hidden = getattr(hook_output, "last_hidden_state", None)
                    if last_hidden is None and isinstance(hook_output, (tuple, list)) and len(hook_output) > 0:
                        last_hidden = hook_output[0]
                    if last_hidden is not None:
                        captured_last_hidden_state["value"] = last_hidden

                hook_handle = base_module.register_forward_hook(_capture_base_model_output)

            try:
                outputs = self.actor.actor_module(**model_kwargs)
            finally:
            ...
            if hidden_states is None and isinstance(outputs, (tuple, list)):
                # Some wrapped HF/FSDP modules ignore return_dict and return a tuple.
                for item in outputs:
                    if isinstance(item, (tuple, list)) and len(item) > 0 and torch.is_tensor(item[-1]):
                        hidden_states = item
                        break
            ...
            if last_hidden_state is None:
                raise RuntimeError("Actor model did not return hidden states for V11 embeddings")

            seq_lens = attention_mask.long().sum(dim=-1).sub(1).clamp(min=0)
            batch_indices = torch.arange(last_hidden_state.shape[0], device=last_hidden_state.device)
            next_token_embeddings = last_hidden_state[batch_indices, seq_lens.to(last_hidden_state.device)]
            next_token_embeddings = next_token_embeddings.detach().float()

        output = DataProto.from_dict(tensors={"next_token_embeddings": next_token_embeddings})
        output = output.to("cpu")
        ...
        if self._is_offload_param:
            offload_fsdp_model_to_cpu(self.actor_module_fsdp)
            log_gpu_memory_usage("After offload actor model during compute_next_token_embeddings", logger=logger)

        return output

    @register(dispatch_mode=Dispatch.DP_COMPUTE_PROTO)
    def compute_v13_repr_and_grad_sketch(self, data: DataProto):
        """
        V13 auxiliary RPC — true per-sample parameter gradient sketch.

        Computes per-sample gradients over lm_head + last transformer block,
        then projects to a low-dimensional sketch via block-wise Rademacher
        random projection. Falls back to representation-residual proxy if
        ...
        projection_dim = int(data.meta_info.get("v13_grad_projection_dim", 256)) if data.meta_info else 256
        projection_seed = int(data.meta_info.get("v13_grad_projection_seed", 20260506)) if data.meta_info else 20260506

        input_ids = data.batch["input_ids"]
        attention_mask = data.batch["attention_mask"]
        batch_size = input_ids.shape[0]

        # --- Phase 1: compute next-token embeddings (reuse existing logic) ---
        emb_output = self.compute_next_token_embeddings(data)
        next_token_embeddings = emb_output.batch["next_token_embeddings"].float()

        # --- Phase 2: true per-sample gradient sketch ---
        try:
            grad_sketches = self._compute_true_grad_sketch(
                input_ids=input_ids,
                attention_mask=attention_mask,
            ...
            print(f"[V13-GRAD] True gradient sketch failed, using embedding fallback: {exc}")
            grad_sketches = self._fallback_embedding_sketch(
                next_token_embeddings,
                data.batch.get("v13_target_embeddings", None),
                projection_dim,
                projection_seed,
            )
        ...
        output = DataProto.from_dict(
            tensors={
                "next_token_embeddings": next_token_embeddings.cpu().float(),
                "grad_sketch": grad_sketches.cpu().float(),
            }
        )
        ...
        if self._is_offload_param:
            offload_fsdp_model_to_cpu(self.actor_module_fsdp)
            log_gpu_memory_usage("After offload actor model during compute_v13_repr_and_grad_sketch", logger=logger)

        return output

    def _compute_true_grad_sketch(
        self,
        ...
        """Per-sample gradient sketch over lm_head + last transformer block."""
        from torch.distributed.fsdp import FSDP

        device = input_ids.device
        actor_module = self.actor.actor_module

        # Determine the unwrapped module for parameter identification
        unwrapped = self._get_unwrapped_actor_module(actor_module)
        model_config = getattr(unwrapped, "config", None)
        num_layers = getattr(model_config, "num_hidden_layers", None)
        if num_layers is None:
            raise RuntimeError("Cannot determine num_hidden_layers from model config")

        last_layer_prefix = f"model.layers.{num_layers - 1}."

        # Temporarily disable gradient checkpointing if enabled
        gc_enabled = getattr(unwrapped, "gradient_checkpointing", False)
        if gc_enabled:
        ...
        try:
            with FSDP.summon_full_params(actor_module, writeback=False, recurse=True):
                # Re-resolve unwrapped module inside summon context
                unwrapped_inner = self._get_unwrapped_actor_module(actor_module)

                # Identify target parameters
                target_param_names = []
                target_params = []
                for name, param in unwrapped_inner.named_parameters():
                    if "lm_head" in name or name.startswith(last_layer_prefix):
                        target_param_names.append(name)
                        target_params.append(param)

                if not target_params:
                    raise RuntimeError(f"No target params found for lm_head or {last_layer_prefix}")

                total_param_count = sum(p.numel() for p in target_params)

                # Save and set requires_grad state
                ...
                try:
                    ...
                    for i in range(batch_size):
                        ...
                        if position_ids is not None:
                            model_kwargs["position_ids"] = position_ids[i:i+1]

                        outputs = unwrapped_inner(**model_kwargs)
                        logits = outputs.logits  # [1, seq_len, vocab_size]

                        # Last valid token position
                        seq_len = int(attention_mask[i].long().sum().item()) - 1
                        seq_len = max(0, seq_len)
                        last_logit = logits[0, seq_len]  # [vocab_size]

                        # Target: next token if available, else self-prediction
                        if seq_len + 1 < input_ids.shape[1] and attention_mask[i, seq_len + 1] == 1:
                            target_token = input_ids[i, seq_len + 1]
                        else:
                            target_token = input_ids[i, seq_len]

                        loss = torch.nn.functional.cross_entropy(
                            last_logit.unsqueeze(0), target_token.unsqueeze(0)
                        )

                        grads = torch.autograd.grad(
                            loss, target_params, retain_graph=False, allow_unused=True
                        )

                        sketch = self._project_grads_to_sketch(
                            grads, total_param_count, projection_dim, projection_seed
                        )
                        grad_sketches.append(sketch)

                        del outputs, logits, last_logit, loss, grads

                    # Single cache clear after all samples
                    get_torch_device().empty_cache()
        ...
        return torch.stack(grad_sketches, dim=0)  # [B, projection_dim]

    def _get_unwrapped_actor_module(self, actor_module):
        """Get the underlying HF model without FSDP/wrapper layers."""
        module = actor_module
        if hasattr(module, "_fsdp_wrapped_module"):
            module = module._fsdp_wrapped_module
        elif hasattr(module, "module"):
            module = module.module
        # For verl's ActorModel wrapper
        if hasattr(module, "model") and hasattr(getattr(module, "model", None), "config"):
            inner = module.model
            if hasattr(inner, "config") and hasattr(inner.config, "num_hidden_layers"):
                return inner
        return module

    def _project_grads_to_sketch(
        self,
        grads: tuple,
        ...
        """Block-wise Rademacher random projection on GPU."""
        BLOCK_SIZE = 65536
        device = next((g.device for g in grads if g is not None), torch.device("cpu"))
        sketch = torch.zeros(projection_dim, dtype=torch.float32, device=device)
        scale = 1.0 / (total_param_count ** 0.5)
        ...
        for grad in grads:
            if grad is None:
                continue
            flat_grad = grad.detach().flatten().float()
            for start in range(0, flat_grad.shape[0], BLOCK_SIZE):
                end = min(start + BLOCK_SIZE, flat_grad.shape[0])
                block = flat_grad[start:end]
                signs_cpu = torch.randint(
                    0, 2, (end - start, projection_dim),
                    generator=rng, dtype=torch.float32
                ) * 2.0 - 1.0
                signs = signs_cpu.to(device, non_blocking=True)
                sketch.add_((block @ signs) * scale)
                del signs, signs_cpu
            del flat_grad

        return sketch.cpu()

    def _fallback_embedding_sketch(
        self,
        ...
        """Fallback: representation-residual projection (same as old v13)."""
        if target_embeddings is not None:
            target_embeddings = target_embeddings.to(next_token_embeddings.device).float()
            if target_embeddings.shape == next_token_embeddings.shape:
                sketch_source = next_token_embeddings - target_embeddings
            else:
                sketch_source = next_token_embeddings
        else:
            sketch_source = next_token_embeddings

        hidden_size = int(sketch_source.shape[-1])
        ...
        projection = projection / max(1.0, hidden_size ** 0.5)
        return sketch_source.cpu().float() @ projection
...
class RewardModelWorker(Worker):
    ...
    def compute_rm_score(self, data: DataProto):
        ...
        with self.ulysses_sharding_manager:
            ...
            # Define mapping function: >20 -> 1, <-20 -> -1, else sigmoid to [-1, 1]
            def map_score(score):
                """
                Map score to [0, 1]:
                - If score > 20: return 1.0
                - If score < -20: return 0
                - Otherwise: use sigmoid to map [-20, 20] to [0, 1]
                """
                # For values outside [-20, 20], directly set to 1 or 0
                result = torch.where(
                    score > 20.0,
                    torch.tensor(1.0, dtype=score.dtype, device=score.device),
                    torch.where(
                        score < -20.0,
                        torch.tensor(0, dtype=score.dtype, device=score.device),
                        # Use sigmoid to map [-20, 20] to [-1, 1]
                        # 2 * sigmoid(x/5) - 1 maps [-20, 20] to approximately [-0.96, 0.96]
                        torch.sigmoid(score / 5.0),
                    ),
                )
                return result

            scores = map_score(scores)
            scores = scores.to(torch.float32)

            # 将math code的结果设置为0
            data_source_list = data.non_tensor_batch["data_source"]
            mask = torch.tensor(
                [source in ['math', 'math_miromind', 'code'] for source in data_source_list],
                dtype=torch.bool,
                device=get_device_id()
            )

            invalid_indices = torch.nonzero(mask, as_tuple=True)[0]
            scores[invalid_indices] = torch.zeros(len(invalid_indices), device=get_device_id(), dtype=torch.float32)
