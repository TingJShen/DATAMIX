...
def get_fsdp_wrap_policy(module, config=None, is_lora=False):
    ...
    explicit_transformer_cls_names_to_wrap = _get_attr("transformer_layer_cls_to_wrap", None)
    fsdp_transformer_layer_cls_to_wrap = explicit_transformer_cls_names_to_wrap or default_transformer_cls_names_to_wrap
    ...
    if min_num_params > 0:
        ...
        if not transformer_cls_to_wrap or explicit_transformer_cls_names_to_wrap is not None:
            raise Exception("Could not find the transformer layer class to wrap in the model.")
        if missing_layer_classes:
            # Some text-only models inherit multimodal no-split entries; skip absent blocks.
            print(f"Skipping missing FSDP wrap classes from model defaults: {missing_layer_classes}")
