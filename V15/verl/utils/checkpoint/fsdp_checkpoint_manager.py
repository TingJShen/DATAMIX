    ...
    def save_checkpoint(self, local_path: str, hdfs_path: str = None, global_step: int = 0, max_ckpt_to_keep=None):
        ...
        if self.rank == 0:
            ...
            if unwrap_model.can_generate() and hasattr(model_config, "name_or_path") and model_config.name_or_path:
                # Some base models do not ship generation_config.json. Fall back to a
                # model-config-derived GenerationConfig so checkpoint saving stays robust.
                try:
                    generation_config = GenerationConfig.from_pretrained(model_config.name_or_path)
                except OSError as exc:
                    log_with_rank(
                        f"generation_config.json not found for {model_config.name_or_path}; using model config fallback: {exc}",
                        rank=self.rank,
                        logger=logger,
                    ...
                    try:
                        ...
                        log_with_rank(
                            f"Skipping generation config save after fallback failure: {fallback_exc}",
                            rank=self.rank,
                            logger=logger,
                            log_only_rank_0=True,
                        )
