...
class FSDPVLLMShardingManager(BaseShardingManager):
    ...
    def update_params(self, updated_params, peft_config=None):
        ...
        # Only remap Qwen3.5 language_model.* keys when the rollout engine has
        # been explicitly forced into the text-only Qwen3 backend.  Native
        # Qwen3.5 ConditionalGeneration expects language_model.* keys directly.
        force_qwen35_text_vllm = os.getenv("ARCHER_QWEN35_FORCE_TEXT_VLLM", "0") == "1"
        should_remap_qwen35_to_text = (
            model_type == "qwen3"
            or "qwen3forcausallm" in model_class
            or (force_qwen35_text_vllm and (model_type.startswith("qwen3_5") or "qwen3" in model_class))
        )
        if should_remap_qwen35_to_text:
            def normalize_qwen35_key(name: str):
                if name.startswith(("visual.", "model.visual.", "mtp.", "model.mtp.")):
                    return None
            ...
            updated_params = {
                ...
                if (normalized_name := normalize_qwen35_key(name)) is not None
            }
