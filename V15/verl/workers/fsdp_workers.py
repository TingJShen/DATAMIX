...
class ActorRolloutRefWorker(Worker):
    ...
    def compute_next_token_embeddings(self, data: DataProto):
        ...
        with torch.no_grad():
            ...
            model_type = str(getattr(self.actor_model_config, "model_type", "")).lower()
            if "qwen" in model_type:
                # V13 only needs the final hidden state. Qwen models otherwise
                # materialize full-sequence logits, which is prohibitively large
                # for 1024-token prompt batches.
    ...
    def compute_v13_repr_and_grad_sketch(self, data: DataProto):
        ...
        The trainer consumes a fixed-size sketch for sample-level Taylor
        scoring. This patch keeps the RPC local to the V13 workspace and uses a
        deterministic projection of the representation residual as the default
        sketch. The trainer can run even when an environment has not enabled a
        heavier per-parameter gradient implementation.
        """
        output = self.compute_next_token_embeddings(data)
        next_token_embeddings = output.batch["next_token_embeddings"].float()

        target_embeddings = data.batch.get("v13_target_embeddings", None)
        ...
        grad_sketch = sketch_source.cpu().float() @ projection

        output = DataProto.from_dict(
            tensors={
                "next_token_embeddings": next_token_embeddings.cpu().float(),
                "grad_sketch": grad_sketch.cpu().float(),
            }
        )
        return output
